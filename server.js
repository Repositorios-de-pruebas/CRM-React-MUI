...
app.post('/login', (req, res, next) => {
  passport.authenticate('local', (err, user, info) => {
    ...
    if (!user) {
      return res.status(401).send(info.message); // Por ejemplo: "Wrong password" o "User not found"
    }
    req.logIn(user, (err) => {
      if (err) {
...
app.get('/logout', (req, res) => {
  console.log("Logout request received");  // Log para verificar la recepción de la solicitud
  req.logout((err) => {
    if (err) {
      console.error("Error logging out:", err); // Para depurar
      return res.status(500).json({ message: "Logout failed" });
    }
    console.log("Logout successful");  // Log para confirmar logout exitoso
    res.clearCookie('connect.sid');  // Asegúrate de que el nombre de la cookie es correcto
